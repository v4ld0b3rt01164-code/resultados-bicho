import { RESULTADOFACIL_GO, USER_AGENT } from '../constantes.js';

const HORARIOS_PADRAO_MIN = 20;
const HORARIOS_EXCECAO_MIN = { 19: 30 };

function grupoPorMilhar(milhar) {
  const s = String(milhar).padStart(4, '0');
  const final = parseInt(s.slice(-2), 10);
  if (final === 0 || final >= 97) return 25;
  return Math.floor((final - 1) / 4) + 1;
}

function extrairDataDoHTML(html) {
  const m = html.match(/<title[^>]*>.*?(\d{2})\/(\d{2})\/(\d{4}).*?<\/title>/i);
  if (m) return `${m[3]}-${m[2]}-${m[1]}`;
  return null;
}

function normalizarHorario(extracao) {
  const match = extracao.match(/(\d{1,2}):(\d{2})/);
  if (match) return `${match[1].padStart(2, '0')}:${match[2]}:00`;
  const matchH = extracao.match(/(\d{1,2})h/i);
  if (matchH) return `${matchH[1].padStart(2, '0')}:00:00`;
  return null;
}

function horarioPadrao(slug, horario) {
  const h = parseInt(horario.slice(0, 2), 10);
  if (Number.isNaN(h)) return horario;
  const min = HORARIOS_EXCECAO_MIN[h] ?? HORARIOS_PADRAO_MIN;
  return `${String(h).padStart(2, '0')}:${String(min).padStart(2, '0')}:00`;
}

function computarComplementos(premiosBase) {
  const premios = premiosBase.map((m, i) => ({
    posicao: `${i + 1}º`,
    milhar: m,
    grupo: grupoPorMilhar(m),
  }));

  const soma = premiosBase.reduce((acc, v) => acc + parseInt(v, 10), 0);
  const m6 = String(soma).padStart(4, '0');
  premios.push({ posicao: '6º', milhar: m6, grupo: grupoPorMilhar(m6) });

  const prod7 = parseInt(premiosBase[0], 10) * parseInt(premiosBase[1], 10);
  const m7 = String(prod7).padStart(9, '0').slice(-6, -3);
  premios.push({ posicao: '7º', milhar: m7, grupo: grupoPorMilhar(m7) });

  const prod8 = parseInt(premiosBase[0], 10) * 4;
  const m8 = String(prod8).slice(-4).padStart(4, '0');
  premios.push({ posicao: '8º', milhar: m8, grupo: grupoPorMilhar(m8) });

  return premios;
}

function extrairResultadoFacilGO(html) {
  html = html.replace(/<a[\s\S]*?<\/a>/gi, '');
  const resultados = [];
  const blocos = html.split(/<h3[^>]*>/gi);
  for (let i = 1; i < blocos.length; i++) {
    const cabecalho = blocos[i].split('</h3>')[0].replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
    if (/federal/i.test(cabecalho)) continue;

    let slug = null;
    let extracao = null;
    if (/LOOK - GOIÁS/.test(cabecalho)) {
      if (!cabecalho.includes('1º ao 5º')) continue;
      slug = 'look-go';
    } else if (/BOA SORTE - GOIÁS/.test(cabecalho)) {
      slug = 'boa-sorte-go';
    } else {
      continue;
    }

    const partes = cabecalho.split(',');
    if (partes.length < 2) continue;
    extracao = `${partes[0].trim()} ${partes[1].trim()}`;
    const horario = normalizarHorario(extracao);
    if (!horario) continue;

    const tbodyMatch = blocos[i].match(/<tbody>([\s\S]*?)<\/tbody>/i);
    if (!tbodyMatch) continue;
    const linhas = tbodyMatch[1].match(/<tr[^>]*>[\s\S]*?<\/tr>/gi) || [];
    const milhares = [];
    for (const linha of linhas) {
      if (linha.includes('soma') || linha.includes('mult')) continue;
      const td = linha.match(/<td[^>]*>(\d{4})<\/td>/i);
      if (td && milhares.length < 5) milhares.push(td[1]);
    }
    if (milhares.length < 5 || milhares[0] === '0000') continue;

    const horarioFinal = slug === 'look-go' ? horarioPadrao(slug, horario) : horario;
    resultados.push({
      slug,
      horario: horarioFinal,
      premios: computarComplementos(milhares),
      primeiro_premio: milhares[0],
      primeiro_grupo: grupoPorMilhar(milhares[0]),
    });
  }
  return resultados;
}

export async function capturarGO(env, data) {
  const res = await fetch(RESULTADOFACIL_GO, {
    headers: { 'User-Agent': USER_AGENT },
  });
  if (!res.ok) throw new Error(`GO HTTP ${res.status}`);
  const html = await res.text();
  const dataPagina = extrairDataDoHTML(html);
  if (!dataPagina) throw new Error('GO data não encontrada');

  const porSlug = { 'look-go': [], 'boa-sorte-go': [] };
  for (const r of extrairResultadoFacilGO(html)) {
    porSlug[r.slug].push({
      data: dataPagina,
      horario: r.horario,
      primeiro_premio: r.primeiro_premio,
      primeiro_grupo: r.primeiro_grupo,
      premios: r.premios,
      fonte: 'scrape',
    });
  }
  return porSlug;
}